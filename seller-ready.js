...
const label=document.createElement('label');label.className='field full';label.innerHTML='Username（与 Shop ID 二选一）<input id="username" placeholder="例如 xxxxx.ph" autocomplete="off" spellcheck="false">';legacy.after(label);
...
const button=document.createElement('button');button.type='button';button.className='btn shop-query-compact';button.textContent='查询店铺资格';hint.after(button);
let result=null,version=0,timer;
showLookup=()=>{const p=$('shopLookup');p.className='lookup-panel';p.textContent=result?.matched?'已精准匹配 '+result.username+' · '+result.site+'。名单资格与免费期实际开通请分别核验。':({empty:'填写 Shop ID 或 Username，查询 Free Trial 名单资格。',invalid:'请检查完整的店铺 ID 或 Username。',not_found:'未找到完全一致的店铺，请核对拼写或联系客户经理。',conflict:'两项未匹配到同一家店铺，请核对或只填写一项。',ambiguous:'存在重复店铺记录，请同时填写 Shop ID 和 Username 确认。',loading:'正在核验店铺…',error:'查询暂不可用，请稍后重试。'})[result?.status||'empty'];};
lookupShop=async()=>{clearTimeout(timer);const seq=++version,shopId=$('shop').value.trim(),username=$('username').value.trim();result={status:'loading'};shopState={status:'empty',matched:false,site:null,ggpStatus:'not_entered'};showLookup();button.disabled=true;try{if(!shopId&&!username)result={status:'empty'};else{const r=await fetch('/api/free-trial',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({shopId,username}),signal:AbortSignal.timeout(12000)});if(!r.ok)throw Error('failed');const d=await r.json();if(seq!==version)return;result=d;}if(result.matched){shopState={...result,status:'ready',ggpStatus:'not_entered'};$('site').value=result.site;}}catch{if(seq!==version)return;result={status:'error'};}finally{if(seq===version){button.disabled=false;showLookup();$('site').dispatchEvent(new Event('input',{bubbles:true}));}}};
button.onclick=lookupShop;
function changed(){version++;clearTimeout(timer);result={status:'empty'};shopState={status:'empty',matched:false,site:null,ggpStatus:'not_entered'};button.disabled=false;showLookup();$('site').dispatchEvent(new Event('input',{bubbles:true}));timer=setTimeout(lookupShop,500)}
